...
import type { PendingUserResponse, Transaction, User, UserResponse } from "@/types"
...
export const adminApi = {
  ...
  getAdminUsers: async ({ page = 1, limit = 10 }: { page?: number; limit?: number } = {}): Promise<UserResponse> => {
    return api.get(`/admin/users`, { params: { page, limit } })
  },

  getUserDetails: async (userId: string): Promise<UserResponse> => {
    return api.get(`/admin/users/${userId}`)
  },

  getPendingPortalAccessUsers: async (): Promise<PendingUserResponse> => {
    return api.get(`/auth/pending-portal-access`)
  },

  updatePortalAccess: async (userId: string, isPortalAccess: boolean): Promise<any> => {
    return api.put(`/auth/users/${userId}/portal-access`, {
      isPortalAccess
    })
  },

  bulkApprovePortalAccess: async (userIds: string[], isPortalAccess: boolean): Promise<any> => {
    return api.post(`/auth/bulk-approve-portal-access`, {
      userIds,
      isPortalAccess
    })
  },
