...
export function DashboardWalletOverview() {
  const { wallet, transactions, isLoading, transactionsLoading, transactionsError } = useAppSelector((state) => state.wallet)

  if (isLoading || transactionsLoading || !wallet) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>
  ...
  // Filter valid transactions from the last 7 days
  const recentTransactions = Array.isArray(transactions) 
    ? transactions 
    : []

  // Calculate totals with proper type checking
  const depositTransactions = recentTransactions.filter(tx => tx?.transactionType === "DEPOSIT")

  const sendTransactions = recentTransactions.filter(tx => tx?.transactionType === "WITHDRAWAL")

  const totalDeposits = depositTransactions
    .reduce((sum, tx) => sum + (Number(tx?.amount) || 0), 0)

  const totalSends = sendTransactions
    .reduce((sum, tx) => sum + (Number(tx?.amount) || 0), 0)
